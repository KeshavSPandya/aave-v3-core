...
library LiquidationLogic {
  ...
  function executeLiquidationCall(
    ...
    (
      vars.collateralKToken,
      vars.collateralPriceSource,
      vars.debtPriceSource,
      vars.liquidationBonus
    ) = _getConfigurationData(eModeCategories, collateralReserve, params);

    vars.userCollateralBalance = vars.collateralKToken.balanceOf(params.user);
    ...
    if (params.receiveAToken) {
      _liquidateKTokens(reservesData, reservesList, usersConfig, collateralReserve, params, vars);
    } else {
      _burnCollateralKTokens(collateralReserve, params, vars);
    }
    ...
    if (vars.liquidationProtocolFeeAmount != 0) {
      ...
      uint256 scaledDownUserBalance = vars.collateralKToken.scaledBalanceOf(params.user);
      // To avoid trying to send more kTokens than available on balance, due to 1 wei imprecision
      if (scaledDownLiquidationProtocolFee > scaledDownUserBalance) {
        vars.liquidationProtocolFeeAmount = scaledDownUserBalance.rayMul(liquidityIndex);
      }
      vars.collateralKToken.transferOnLiquidation(
        params.user,
        vars.collateralKToken.RESERVE_TREASURY_ADDRESS(),
        vars.liquidationProtocolFeeAmount
      );
    }
    ...
    IERC20(params.debtAsset).safeTransferFrom(
      msg.sender,
      vars.debtReserveCache.kTokenAddress,
      vars.actualDebtToLiquidate
    );

    IKToken(vars.debtReserveCache.kTokenAddress).handleRepayment(
      msg.sender,
      params.user,
      vars.actualDebtToLiquidate
    );
    ...
    emit LiquidationCall(
      ...
      vars.actualCollateralToLiquidate,
      msg.sender,
      params.receiveKToken
    );
  }

  /**
   * @notice Burns the collateral kTokens and transfers the underlying to the liquidator.
   * @dev   The function also updates the state and the interest rate of the collateral reserve.
   * @param collateralReserve The data of the collateral reserve
   * @param params The additional parameters needed to execute the liquidation function
   * @param vars The executeLiquidationCall() function local vars
   */
  function _burnCollateralKTokens(
  ...
  /**
   * @notice Liquidates the user kTokens by transferring them to the liquidator.
   * @dev   The function also checks the state of the liquidator and activates the kToken as collateral
  ...
  function _liquidateKTokens(
    ...
    uint256 liquidatorPreviousATokenBalance = IERC20(vars.collateralKToken).balanceOf(msg.sender);
    vars.collateralKToken.transferOnLiquidation(
      params.user,
      msg.sender,
      vars.actualCollateralToLiquidate
    ...
    if (liquidatorPreviousATokenBalance == 0) {
      ...
      if (
        ValidationLogic.validateAutomaticUseAsCollateral(
          ...
          collateralReserve.kTokenAddress
        )
      ) {
  ...
  function _getConfigurationData(
    ...
    DataTypes.ReserveData storage collateralReserve,
    DataTypes.ExecuteLiquidationCallParams memory params
  ) internal view returns (IKToken, address, address, uint256) {
    IKToken collateralKToken = IKToken(collateralReserve.kTokenAddress);
    ...
    return (collateralKToken, collateralPriceSource, debtPriceSource, liquidationBonus);
  }
